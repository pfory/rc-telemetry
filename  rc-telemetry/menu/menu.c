...
uint8_t lastKey=0;
int menuIndex=0;
uint8_t ulozData=0xff;

void foo(int *uk, uint8_t op, int Lval, int Hval) {
}
...
void resetVar(int *uk, uint8_t op, int Lval, int Hval) {
	*uk=Lval;
}

void setChar(int *uk, uint8_t op, int Lval, int Hval) {
	ulozData=10;
	if (op==1) {
		char zn=(char)*uk;
		zn++;
...
void zpracuj() {
	...
	if (bit_is_clear(keyUPPort,keyUPPin)) {
		...
		pf[menuIndex](menuVar[menuIndex], 1, menuRozsah[menuIndex][0], menuRozsah[menuIndex][1]);
	}
		...
		pf[menuIndex](menuVar[menuIndex], 0, menuRozsah[menuIndex][0], menuRozsah[menuIndex][1]);
	}
